...
My previous program {\mc XCCDC1} was derived directly from hers, by
adding new mechanisms to enforce domain consistency.

This program also experiments in another direction, by modifying the
branching heuristic as suggested by
Boussemart, Hemery, Lecoutre, and Sais in {\sl Proc.\ 16 European
Conference on Artificial Intelligence} (2004), 146--150. My former
...
As in {\mc XCCDC1}, I confess in advance that
the concepts below might not be easy to grasp. Some of the interactions are
rather subtle, and they're just beginning to make sense to me as I put the pieces
together. Hopefully all will become clear
by the time I finish! I've retained the documentation of {\mc SSXCC2}'s
...
@d show_mstats 4096 /* |vbose| code to show memory usage in key arrays */
@d show_option_counts 8192 /* |vbose| code to count active options */
...
if (k) {
  fprintf(stderr, "Usage: "O"s [v<n>] [m<n>] [d<n>] [c<n>] [C<n>]"
       " [l<n>] [t<n>] [T<n>] [w<f>] [W<f>] [S<bar>] [x<n>] [X<bar>] foo.dlx\n",
                            argv[0]);
  exit(-1);
}
...
|wt(x)| occupies the position of |match(x)| in a secondary item, namely |set(x-6)|,
but |wt(x)| is a floating-point quantity.
This weight, initially~1.0, is increased by |dw| whenever we run into
a situation where |x| cannot be supported.
...
@d mark(x) set[(x)-5].i /* a stamp for incompatible items */
@d match(x) set[(x)-6].i /* a required color in compatibility tests */
@d wt(x) set[(x)-6].f /* the current weight of item |x| */
...
@#
typedef union {
  int i; /* an integer (32 bits, ergo 4 bytes) */
...
tetrabyte set[max_nodes+6*max_cols]; /* the sets of active options for active items */
...
for (k=0,j=6;k<itemlength;k++)
  oo,item[k]=j,j+=6+size((k+1)<<2);
setlength=j-5;
...
newlevel: nodes++;
  ...
  oo,choice[level]=cur_choice=set[best_itm].i;
...
purgeit:@+if (vbose&show_option_counts) totopts=levelopts[level];
  ...
  if (!(o,purge_the_option(choice[level],active))) {
    @<Increase the weight of |tough_itm|@>;
    goto backup;
  }
  if (!empty_the_queue()) {
