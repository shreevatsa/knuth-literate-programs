...
\def\losub#1{^{\vphantom+}_{#1}} % for contexts like $x^+_n+x\losub n$
@s mod and
\let\Xmod=\bmod % this is CWEB magic for using "mod" instead of "%"
@i gb_types.w

@*Intro. This program finds all the Hamiltonian cycles of a given digraph.
...
until they are all linked together.
I've based it on {\mc SSHAM}, which is similar but for undirected graphs.

(As noted in {\mc SSHAM}, the basic idea was introduced by Geoffrey Selby
in 1970. Selby presented it for undirected graphs, but observed that
directed graphs could be handled in a similar way. Such an adaptation
was made by his advisor, Nicos Christofides, in Section 10.2.3 of
Christofides's book {\sl Graph Theory\/} (1975); Silvano Martello extended it
in 1983
...
but did not implement a digraph variant until now.)

This program, like {\mc SSXCC}, reports the running time
...
@d maxn 1000 /* at most this many vertices in the digraph */
@d infty (2*maxn) /* larger than any vertex number in our graph */
...
int mind; /* smallest indegree or outdegree in the given graph */
...
a digraph~|g| and removing arcs until only an oriented cycle is left. 

In 1972, R. M. Karp devised a simple way to convert any digraph $D$
to an undirected graph~$G$,
where $D$ and~$G$ contain exactly the same number of Hamiltonian cycles:
Each vertex~$v$ of~$D$ leads to three vertices $\{v^-,v,v^+\}$ of~$G$;
and each arc $u\dadj v$ of~$D$ leads to an edge $u^-\adj v^+$ of~$G$.
The graph~$G$ also contains two edges $v^-\adj v\adj v^+$ for each
vertex~$v$ of~$D$. Clearly $D$ contains the oriented cycle
$v_0\dadj v_1\dadj\cdots\dadj v_n=v_0$ if and only if $G$ contains the cycle
$$v_0^+\adj v\losub0\adj v_0^-\adj v_1^+\adj v\losub1\adj v_1^-\adj \cdots\adj
v_n^+=v_0^+.$$

With that construction, we could reduce the digraph problem faced by
{\mc SSDIHAM} to the graph problem already solved by {\mc SSHAM}.
But we can do better: (i) We need only work with the vertices $v^-$ and~$v^+$,
not with~$v$; and (ii) we can force the arc $u\dadj v$ (or, equivalently,
the edge $u^-\adj v^+$) to be in the cycle whenever either $u^-$ or~$v^+$
has degree~1 in the remaining graph.

Thus we shall regard the user's $n$-vertex digraph as a $2n$-vertex
graph~|g|, as we carry out the computation.
Our undirected~|g| is bipartite, with $n$ vertices in each part.
And our goal is to find all perfect matchings of~|g| that correspond
to a single cycle, when the $n$ (nonexistent) edges $v^-\adj v^+$
are added to that matching.

We use a sparse-set representation for |g|, because such structures provide
...
Vertices $v^-$ and $v^+$ are represented internally by the respective
integers $2v$ and $2v+1$, where $0\le v<|nn|$.

@<Glob...@>=
int nbr[2*maxn][2*maxn], adj[2*maxn][2*maxn];
  /* sparse-set representation of |g| */
int degree[2*maxn]; /* vertex degree in our graph (for diagnostics only) */
...
treated as two arcs, $u^-\dadj v^+$ and $v^+\dadj u^-$.)
...
|u| and~|v|. If the $k$th chosen edge is $u^-\adj v^+$,
we have either $e[k].u=u^-$ and $e[k].v=v^+$ or
               $e[k].u=v^+$ and $e[k].v=u^-$,
depending on which vertex was branched on or triggered.
...
void print_edges(void) {
  register int k;
  for (k=0;k<eptr;k++)
    printf(""O"s"O"c--"O"s"O"c\n",name(e[k].u),name(e[k].v));
}

@ If the edges chosen so far include a maximal subpath
$$v_0^-\adj v_1^+,\ v_1^-\adj v_2^+, \ \ldots, \ v_{k-1}^-\adj v_k^+,$$
where $v_0^+$ and $v_k^-$ don't participate in any other chosen edge,
we say that $v_0+$ and $v_k^-$ are ``outer'' vertices,
while $\{v_0^-,v_1^+,v_1^-,\ldots,v_{k-1}^-,v_k^+\}$ are ``inner.''
A vertex that's neither
...
above, we have $|mate|(v_0^+)=v_k^-$ and $|mate|(v_k^-)=v_0^+$;
that rule defines
|mate(v)| for all outer vertices~|v|. We also define |mate(v)=-1| if
|v| is bare. The value of |mate(v)| is undefined when |v| is
...
void print_vert(int v) {
  register int k;
  printf(""O"s"O"c:",name(v));
  for (k=0;;k++) {
    if (k==deg(v)) printf("|");@+else printf(" ");
    if (k==degree[v]) break;
    printf(""O"s"O"c",name(nbr[v][k]));
  }
  if (mate(v)<0) printf(" "O"s\n",ivis[v]<visible?"bare":"inner");
  else if (ivis[v]>=visible) printf(" mate "O"s"O"c, inner\n",name(mate(v)));
  else printf(" mate "O"s"O"c\n",name(mate(v)));
}
@#
...
@ @<Glob...@>=
int vis[2*maxn],ivis[2*maxn]; /* sparse-set representation of visibility */
int visible; /* this many vertices are currently visible */

@ @<Initialize@>=
for (k=0;k<2*nn;k++) oo,vis[k]=ivis[k]=k;
visible=2*nn;
...
@ @d head (2*maxn) /* address of the list header in the |act| array */
...
@d basename(v) (g->vertices+iperm[v])->name
@d name(v) basename((v)>>1), ((v)&1?'+':'-') /* two arguments for |printf| */
...
for (mind=infty,u=0;u<2*nn;u++) {
  if (o,deg(u)<mind) mind=deg(u),curv=u;
  if (deg(u)==1) o,trigger[trigptr++]=u;
  for (v=0;v<2*nn;v++) if (adj[u][v]!=infty && adj[v][u]==infty)
    confusion("asymmetry");
}
if (mind<1) {
...
@ @<Set up the |nbr| and |adj| arrays@>=
for (i=0;i<2*nn;i++) for (o,j=0;j<2*nn;j++) o,adj[i][j]=infty;
for (v=0;v<nn;v++) {
  register int up,vp,ud;
  register Arc *a;
  rmems++,vp=perm[v];
  oo; /* mems to fetch |nbr[vp+vp]| and |adj[vp+vp]| in the following loop */
  for (d=0,o,a=(g->vertices+v)->arcs;a;o,a=a->next,d++) {
    ...
    if (adj[vp+vp][up+up+1]!=infty) {
      fprintf(stderr,"graph "O"s has a repeated arc "O"s--"O"s!\n",
             argv[1],(g->vertices+v)->name,(g->vertices+u)->name);
      exit(-4);
    }
    oo,nbr[vp+vp][d]=up+up+1,adj[vp+vp][up+up+1]=d;
    o,ud=deg(up+up+1);
    oo,nbr[up+up+1][ud]=vp+vp;
    oo,adj[up+up+1][vp+vp]=ud;
    o,deg(up+up+1)=degree[up+up+1]=ud+1;
  }
  o,mate(vp+vp)=-1,deg(vp+vp)=degree[vp+vp]=d;
  o,mate(vp+vp+1)=-1;
  if (randomizing) { /* permute the list of neighbors */
    for (j=1;j<d;j++) {
      mems+=4,k=gb_unif_rand(j+1);
      oo,u=nbr[vp+vp][j],w=nbr[vp+vp][k];
      oo,nbr[vp+vp][j]=w,nbr[vp+vp][k]=u;
      oo,adj[vp+vp][w]=j,adj[vp+vp][u]=k;
    }
  }
}
if (randomizing) {
  for (v=0;v<nn;v++) {
    for (o,j=1;j<deg(v+v+1);j++) {
      mems+=4,k=gb_unif_rand(j+1);
      oo,u=nbr[v+v+1][j],w=nbr[v+v+1][k];
      oo,nbr[v+v+1][j]=w,nbr[v+v+1][k]=u;
      oo,adj[v+v+1][w]=j,adj[v+v+1][u]=k;
    }
  }
  mems+=rmems; /* |rmems| are ignored if |perm| is the identity */
}
...
void sanity(void) {
  ...
  for (pv=head,v=act[pv].rlink;v!=head;pv=v,v=act[pv].rlink) {
    if (act[v].llink!=pv) fprintf(stderr,"llink of "O"s"O"c is bad!\n",name(v));
    if (ivis[v]>=visible) fprintf(stderr,"active "O"s"O"c is invisible!\n",name(v));
    u=mate(v);
    if (u<0) fprintf(stderr,"active "O"s"O"c has no mate!\n",name(v));
    else if (u>=2*nn) fprintf(stderr,"active "O"s"O"c has bad mate!\n",name(v));
    else if (mate(u)!=v)
      fprintf(stderr,"mate(mate("O"s"O"c))!="O"s"O"c!\n",name(v),name(v));
    else if (adj[v][u]<deg(v))
      fprintf(stderr,"there's an arc from "O"s"O"c to its mate!\n",name(v));
  }
  if (act[head].llink!=pv) fprintf(stderr,"llink of head is bad!\n");
  for (v=0;v<2*nn;v++) {
    for (k=0;k<degree[v];k++) if (adj[v][nbr[v][k]]!=k)
      fprintf(stderr,"Bad nbr["O"s"O"c]["O"d]!\n",name(v),k);
    for (u=0;u<2*nn;u++) if (adj[v][u]!=infty && nbr[v][adj[v][u]]!=u)
      fprintf(stderr,"Bad adj["O"s"O"c]["O"s"O"c]!\n",name(v),name(u));
    if (ivis[v]<visible && eptr<nn) { /* |v| is outer or bare */
      if (mate(v)<0 && mate(v^1)>=0 && ivis[v^1]<visible)
        fprintf(stderr,""O"s"O"c is half bare!\n",name(v));
      for (k=0;k<deg(v);k++) {
        u=nbr[v][k];
        if (ivis[u]>=visible)
          fprintf(stderr,"inner "O"s"O"c is touched by "O"s"O"c!\n",name(u),name(v));
        else if (adj[u][v]>=deg(u))
          fprintf(stderr,"arc "O"s"O"c to "O"s"O"c is missing!\n",name(u),name(v));
      }
    }
  }
}
...
The |savestack| grows by exactly |2*nn| entries at each level.
...
int actstack[maxn*2*maxn]; /* lists of active vertices at each level */
...
vertex of degree~1 had entered the trigger list.
...
for (o,j=(level?nd[level-1].t:0);j<trigptr;j++) {
  ...
  if (o,ivis[v]>=visible) continue; /* |v| is no longer visible */
  if (deg(v)==0) {
    if (vbose&show_details)
      fprintf(stderr,"oops, no neighbors for "O"s"O"c\n",name(v));
    goto try_again;
  }
  oo,u=nbr[v][0]; /* now |v| is bare and connected only to |u| */
  e[eptr].u=v,e[eptr++].v=u;@+vprint(); /* the |e| array is memfree */
  makeinner(v);
  activate(v^1);
  ...
  o,w=mate(u);
  if (w<0) @<Promote BB to OO@>@;
  else @<Promote BO to OI@>;    
}
      
@ A subtle point ought to be explained here: Suppose the input digraph
...
$0^-$ and $1^+$ to become inner, while $0^+$ and $1^-$ become
outer (and active). Thus $0^+$, $1^-$, and $1^+$ will no longer
be bare, and they won't actually
trigger anything. Moreover, $1^-$ and $0^+$ will be mates; and the edge between
...
for (oo,curv=k=act[head].rlink,d=deg(curv);k!=head;o,k=act[k].rlink) {
  if (vbose&show_details) fprintf(stderr," "O"s"O"c("O"d)",name(k),deg(k));
  if (o,deg(k)<d) curv=k,d=deg(k);
}
if (vbose&show_details)
  fprintf(stderr,", branching on "O"s"O"c("O"d)\n",name(curv),d);
...
@ We use the (interesting) fact that a vertex $u^-$ is bare if and only if
its partner $u^+$ is bare.

@<Choose the edge from |curv| to |nbr[curv][curi]|@>=
o,curu=nbr[curv][curi];
o,curw=mate(curv);
e[eptr++].v=curu;
if (vbose&show_choices) fprintf(stderr,""O"3d: "O"s"O"c--"O"s"O"c ("O"d of "O"d)\n",
...
if (curt<0) { /* |curu| is bare */
  makeinner(curu);
  activate(curu^1); /* the partner of |curu| becomes outer */
  makemates(curu^1,curw); /* and is mated to |curv|'s mate */
}@+else { /* |curu| is outer */
  makemates(curt,curw);
  deactivate(curu);
}
...
void print_state(FILE *stream)
{
  ...
  for (j=l=0;l<=level;j++,l++) {
    while (j<nd[l].m) {
      fprintf(stream,"      "O"s"O"c--"O"s"O"c\n",name(e[j].u),name(e[j].v));
      j++;
    }
    if (l) {
...
@<Unscramble and print the current solution@>=
{
  ...
  for (k=0;k<nn;k++) {
    i=e[k].u,j=e[k].v;
    if (i&1) succ[j>>1]=i>>1;@+else succ[i>>1]=j>>1;
  }
...
@ @<Glob...@>=
int succ[maxn]; /* the successor of a given vertex of the digraph */
...
There's no problem if the graph has at least one vertex of degree 1,
because the |trigger| list will provide us with at least two active vertices
in such a case. But if all vertices have degree 2 or more, we've got to
...
the process until |curv| or some other vertex has only one neighbor left.)

@<Bootstrap...@>=
level=0;
d=mind;
if (d==1) oo,nd[0].v=-1,nd[0].d=d;
else {
  curi=0;
force:@+if (act[head].llink!=head || act[head].rlink!=head) confusion("root");
  oo,curu=nbr[curv][d-1-curi];
  e[0].u=curv,e[0].v=curu,eptr=1;
  if (vbose&show_choices) fprintf(stderr,"  0: "O"s"O"c--"O"s"O"c ("O"d of "O"d)\n",
       name(e[0].u),name(e[0].v),curi+1,d);
  @<Record the current status...@>;
  for (oo,k=deg(curu)-1;k>=0;k--) {
    o,t=nbr[curu][k]; /* the mem for fetching |nbr[curu]| was charged above */
    if (t!=curv) removex(t,curu);
  }
  for (oo,k=deg(curv)-1;k>=0;k--) {
    o,t=nbr[curv][k]; /* the mem for fetching |nbr[curv]| was charged above */
    if (t!=curu) removex(t,curv);
  }
  makeinner(curu);@+makeinner(curv);
  activate(curu^1);@+activate(curv^1);
  makemates(curu^1,curv^1);
} /* we fall through to |advance| */

@ Back at root level, all vertices are again bare.
Since the edge that was previously tried at root level is now no longer
present, one or both of its vertices might now have degree~1;
...
if (deg(curv)==1) trigger[trigptr++]=curv;
