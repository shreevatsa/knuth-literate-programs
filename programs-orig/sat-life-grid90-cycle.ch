...
Moreover, this version assumes that $m=n$ and that the configurations
all have symmetry under 90-degree rotation. (The situation at
$(x,y)$ is identical to the situation at $(n+1-y,x)$.) Thus the
...
@y
  for (tt=0;tt<r;tt++) {
    ymax=nn,ymin=1;
    xmax=mm,xmin=1;
    for (x=0;x+x<=nn+1;x++) for (y=x;y+x<=nn+(x==y);y++) {
      @<If cell $(x,y)$ is obviously dead at time $t+1$, |continue|@>;
      a(x,y);
      zprime(x,y);
      if (pp(x,y)==0)
...
printf("~ sat-life-grid90-cycle %d %d %d\n",mm,nn,r);
@z
@x
...
@y
    y=clause[p]&0xfff;
    if (c==0) while (x>y || y+x>nn+(x==y)) t=x,x=nn+1-y,y=t;
@z
@x
             x,timecode[tt+k],y);
@y
...
@*Index.
@y
@ In particular, this step rules out the case where everything is dead.

@<Rule out smaller periods@>=
for (k=1;k<r;k++) if (r%k==0) { /* generation $k$ shouldn't equal gen 0 */
  for (x=1;x+x<=nn+1;x++) for (y=x;y+x<=nn+(x==y);y++) {
