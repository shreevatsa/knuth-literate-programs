...
The third points directly to the column containing the node.
...
for the rows as well as for the columns. Thus it had two additional
...
accesses in all of the inner loops where rows are being traversed. However,
I~failed to realize that the |col| and |right| fields were both stored in the
...
@ When I learned that the covering of individual columns can be done safely in
