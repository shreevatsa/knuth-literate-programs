...
@y
@d assigns(x) set[(x)-5].f /* number of assignments tried so far for |x| */
@d failrate(x) set[(x)-6].f /* the current ``failure rate'' of |x| */
...
@y
  if (k<=osecond) oo,assigns(j)=1.0,failrate(j)=0.5;
@z
@x
...
other primary variable that it doesn't cover.
...
@y
  for (k=0;k<active;k++) if (o,item[k]<second) {
    ...
    if ((vbose&show_details) &&
      ...
      else fprintf(stderr,"("O"d,"O".1f)",s,w);
    }
  }
@z
@x
